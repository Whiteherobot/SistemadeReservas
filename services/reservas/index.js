...
inventarioClient.breaker.fallback(() => {
  logger.warn('Inventory fallback: Using local cache');
...
async function consultarInventario(eventoId) {
  try {
    ...
    if (cached && (Date.now() - cached.timestamp) < CACHE_TTL) {
      logger.warn(`Using cached inventory (${Math.floor((Date.now() - cached.timestamp) / 1000)}s old)`);
      return { ...cached.data, fromCache: true };
    }
    
    throw new Error('Inventory service unavailable and no cached data');
  }
}
